...
const ElevatorTechnical: React.FC = () => {
  ...
  const route = useRoute<RouteProp<ConstructionStackParamList, 'ElevatorTechnical'>>();
  const {Name} = route.params;

  const [areaElevatorTechnical, setAreaElevatorTechnical] = useState('');
  ...
  const constructionArea = areaElevatorTechnical
    ? Number(areaElevatorTechnical) * coefficient
    : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceElevatorTechnical = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaPIT');
      if (savedArea) {
        setAreaElevatorTechnical(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaElevatorTechnical', areaElevatorTechnical);
    };
    saveArea();
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaElevatorTechnical}
          onChangeText={setAreaElevatorTechnical}
        ...
        <View style={styles.titleGroup}>
          ...
          <Text style={styles.total}>{totalPriceElevatorTechnical.toLocaleString()} VNĐ</Text>
        </View>
      </View>
