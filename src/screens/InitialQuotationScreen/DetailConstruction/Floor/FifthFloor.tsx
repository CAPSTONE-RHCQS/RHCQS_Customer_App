...
const FifthFloor: React.FC = () => {
  const navigationContruction = useNavigation<AppStackNavigationProp>();
  const route = useRoute<RouteProp<ConstructionStackParamList, 'FifthFloor'>>();
  const {Name} = route.params;

  const [areaFifthFloor, setAreaFifthFloor] = useState('');
  ...
  const constructionArea = areaFifthFloor ? Number(areaFifthFloor) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceFifthFloor = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaPIT');
      if (savedArea) {
        setAreaFifthFloor(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaFifthFloor', areaFifthFloor);
    };
    saveArea();
  }, [areaFifthFloor]);
  ...
  const handleContinuePress = async () => {
    ...
    await storage.setItem('totalPriceFifthFloor', totalPriceFifthFloor.toString());
    await storage.setItem('areaFifthFloor', areaFifthFloor.toString());

    navigationContruction.navigate('ConstructionScreen', {
      totalPriceFifthFloor,
      areaFifthFloor: Number(areaFifthFloor),
      source: 'Lầu 5',
    });
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaFifthFloor}
          onChangeText={setAreaFifthFloor}
        ...
        <View style={styles.titleGroup}>
          ...
          <Text style={styles.total}>{totalPriceFifthFloor.toLocaleString()} VNĐ</Text>
        </View>
      </View>
