...
const SixthFloor: React.FC = () => {
  ...
  const route = useRoute<RouteProp<ConstructionStackParamList, 'SixthFloor'>>();
  const {Name} = route.params;

  const [areaSixthFloor, setAreaSixthFloor] = useState('');
  ...
  const constructionArea = areaSixthFloor ? Number(areaSixthFloor) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceSixthFloor = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaPIT');
      if (savedArea) {
        setAreaSixthFloor(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaSixthFloor', areaSixthFloor);
    };
    saveArea();
  }, [areaSixthFloor]);
  ...
  const handleContinuePress = async () => {
    ...
    await storage.setItem('totalPriceSixthFloor', totalPriceSixthFloor.toString());
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaSixthFloor}
          onChangeText={setAreaSixthFloor}
        ...
        <View style={styles.titleGroup}>
          ...
          <Text style={styles.total}>{totalPriceSixthFloor.toLocaleString()} VNĐ</Text>
        </View>
      </View>
