...
const ThirdFloor: React.FC = () => {
  const navigationContruction = useNavigation<AppStackNavigationProp>();
  const route = useRoute<RouteProp<ConstructionStackParamList, 'ThirdFloor'>>();
  const {Name} = route.params;

  const [areaThirdFloor, setAreaThirdFloor] = useState('');
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaThirdFloor', areaThirdFloor);
    };
    saveArea();
  }, [areaThirdFloor]);
  ...
  const handleContinuePress = async () => {
    ...
    navigationContruction.navigate('ConstructionScreen', {
      totalPriceThirdFloor,
      areaThirdFloor: Number(areaThirdFloor),
      source: 'Lầu 3',
    });
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaThirdFloor}
          onChangeText={setAreaThirdFloor}
