...
const OpenRooftop: React.FC = () => {
  const navigationContruction = useNavigation<AppStackNavigationProp>();
  const route = useRoute<RouteProp<ConstructionStackParamList, 'OpenRooftop'>>();
  const {Name} = route.params;

  const [areaOpenRooftop, setAreaOpenRooftop] = useState('');
  ...
  const constructionArea = areaOpenRooftop ? Number(areaOpenRooftop) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceOpenRooftop = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaPIT');
      if (savedArea) {
        setAreaOpenRooftop(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaOpenRooftop', areaOpenRooftop);
    };
    saveArea();
  }, [areaOpenRooftop]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaOpenRooftop}
          onChangeText={setAreaOpenRooftop}
        ...
        <View style={styles.titleGroup}>
          ...
          <Text style={styles.total}>{totalPriceOpenRooftop.toLocaleString()} VNĐ</Text>
        </View>
      </View>
