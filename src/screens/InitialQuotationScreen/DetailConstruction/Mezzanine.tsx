...
const Mezzanine: React.FC = () => {
  ...
  const route = useRoute<RouteProp<ConstructionStackParamList, 'Mezzanine'>>();
  const {Name} = route.params;

  const [areaMezzanine, setAreaMezzanine] = useState('');
  ...
  const constructionArea = areaMezzanine ? Number(areaMezzanine) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceMezzanine = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaPIT');
      if (savedArea) {
        setAreaMezzanine(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaMezzanine', areaMezzanine);
    };
    saveArea();
  }, [areaMezzanine]);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        ...
        <View style={styles.titleGroup}>
          ...
          <Text style={styles.total}>{totalPriceMezzanine.toLocaleString()} VNĐ</Text>
        </View>
      </View>
