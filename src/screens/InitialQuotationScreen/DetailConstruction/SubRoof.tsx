...
const SubRoof: React.FC = () => {
  const navigationContruction = useNavigation<AppStackNavigationProp>();
  const route = useRoute<RouteProp<ConstructionStackParamList, 'SubRoof'>>();
  const {Name} = route.params;

  const [areaSubRoof, setAreaSubRoof] = useState('');
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaSubRoof', areaSubRoof);
    };
    saveArea();
  }, [areaSubRoof]);
