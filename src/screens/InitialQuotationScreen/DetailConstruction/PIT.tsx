...
const PIT: React.FC = () => {
  ...
  const [areaPIT, setAreaPIT] = useState('');
  ...
  const constructionArea = areaPIT ? Number(areaPIT) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPricePIT = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaPIT');
      if (savedArea) {
        setAreaPIT(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaPIT', areaPIT);
    };
    saveArea();
  }, [areaPIT]);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        ...
        <View style={styles.titleGroup}>
          ...
          <Text style={styles.total}>{totalPricePIT.toLocaleString()} VNĐ</Text>
        </View>
      </View>
