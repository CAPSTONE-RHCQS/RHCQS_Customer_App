...
const FifthFloorVoid: React.FC = () => {
  const navigationContruction = useNavigation<AppStackNavigationProp>();
  const route = useRoute<RouteProp<ConstructionStackParamList, 'FifthFloorVoid'>>();
  const {Name} = route.params;

  const [areaFifthFloorVoid, setAreaFifthFloorVoid] = useState('');
  ...
  const constructionArea = areaFifthFloorVoid ? parseFloat(areaFifthFloorVoid) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceFifthFloorVoid = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaFifthFloorVoid');
      if (savedArea) {
        setAreaFifthFloorVoid(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaFifthFloorVoid', areaFifthFloorVoid);
    };
    saveArea();
  }, [areaFifthFloorVoid]);
  ...
  const handleContinuePress = async () => {
    await storage.setItem('totalPriceFifthFloorVoid', totalPriceFifthFloorVoid.toString());
    await storage.setItem('areaFifthFloorVoid', areaFifthFloorVoid.toString());

    navigationContruction.navigate('ConstructionScreen', {
      totalPriceFifthFloorVoid,
      areaFifthFloorVoid: Number(areaFifthFloorVoid),
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaFifthFloorVoid}
          onChangeText={setAreaFifthFloorVoid}
        ...
        <View>
          ...
          <View style={styles.titleGroup}>
            ...
            <Text style={styles.total}>{totalPriceFifthFloorVoid.toLocaleString()} VNĐ</Text>
          </View>
        </View>
      </View>
