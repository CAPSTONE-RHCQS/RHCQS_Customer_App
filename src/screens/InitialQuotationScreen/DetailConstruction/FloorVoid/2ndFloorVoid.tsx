...
const SecondFloorVoid: React.FC = () => {
  const navigationContruction = useNavigation<AppStackNavigationProp>();
  const route = useRoute<RouteProp<ConstructionStackParamList, 'SecondFloorVoid'>>();
  const {Name} = route.params;

  const [areaSecondFloorVoid, setAreaSecondFloorVoid] = useState('');
  ...
  const constructionArea = areaSecondFloorVoid ? parseFloat(areaSecondFloorVoid) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceSecondFloorVoid = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaSecondFloorVoid');
      if (savedArea) {
        setAreaSecondFloorVoid(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaSecondFloorVoid', areaSecondFloorVoid);
    };
    saveArea();
  }, [areaSecondFloorVoid]);
  ...
  const handleContinuePress = async () => {
    await storage.setItem('totalPriceSecondFloorVoid', totalPriceSecondFloorVoid.toString());
    await storage.setItem('areaSecondFloorVoid', areaSecondFloorVoid.toString());

    navigationContruction.navigate('ConstructionScreen', {
      totalPriceSecondFloorVoid,
      areaSecondFloorVoid: Number(areaSecondFloorVoid),
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaSecondFloorVoid}
          onChangeText={setAreaSecondFloorVoid}
        ...
        <View>
          ...
          <View style={styles.titleGroup}>
            ...
            <Text style={styles.total}>{totalPriceSecondFloorVoid.toLocaleString()} VNĐ</Text>
          </View>
        </View>
      </View>
...
export default SecondFloorVoid;
