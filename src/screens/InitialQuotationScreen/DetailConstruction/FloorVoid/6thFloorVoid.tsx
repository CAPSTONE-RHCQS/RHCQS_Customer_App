...
const SixthFloorVoid: React.FC = () => {
  const navigationContruction = useNavigation<AppStackNavigationProp>();
  const route =
    useRoute<RouteProp<ConstructionStackParamList, 'SixthFloorVoid'>>();
  const {Name} = route.params;

  const [areaSixthFloorVoid, setAreaSixthFloorVoid] = useState('');
  ...
  const constructionArea = areaSixthFloorVoid
    ? parseFloat(areaSixthFloorVoid) * coefficient
    : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceSixthFloorVoid = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaSixthFloorVoid');
      if (savedArea) {
        setAreaSixthFloorVoid(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaSixthFloorVoid', areaSixthFloorVoid);
    };
    saveArea();
  }, [areaSixthFloorVoid]);
  ...
  const handleContinuePress = async () => {
    await storage.setItem(
      ...
      totalPriceSixthFloorVoid.toString(),
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaSixthFloorVoid}
          onChangeText={setAreaSixthFloorVoid}
        ...
        <View>
          ...
          <View style={styles.titleGroup}>
            ...
            <Text style={styles.total}>
              {totalPriceSixthFloorVoid.toLocaleString()} VNĐ
            </Text>
          </View>
        </View>
      </View>
