...
const FourthFloorVoid: React.FC = () => {
  const navigationContruction = useNavigation<AppStackNavigationProp>();
  const route = useRoute<RouteProp<ConstructionStackParamList, 'FourthFloorVoid'>>();
  const {Name} = route.params;

  const [areaFourthFloorVoid, setAreaFourthFloorVoid] = useState('');
  ...
  const constructionArea = areaFourthFloorVoid ? parseFloat(areaFourthFloorVoid) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceFourthFloorVoid = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaFourthFloorVoid');
      if (savedArea) {
        setAreaFourthFloorVoid(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaFourthFloorVoid', areaFourthFloorVoid);
    };
    saveArea();
  }, [areaFourthFloorVoid]);
  ...
  const handleContinuePress = async () => {
    await storage.setItem('totalPriceFourthFloorVoid', totalPriceFourthFloorVoid.toString());
    await storage.setItem('areaFourthFloorVoid', areaFourthFloorVoid.toString());

    navigationContruction.navigate('ConstructionScreen', {
      totalPriceFourthFloorVoid,
      areaFourthFloorVoid: Number(areaFourthFloorVoid),
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaFourthFloorVoid}
          onChangeText={setAreaFourthFloorVoid}
        ...
        <View>
          ...
          <View style={styles.titleGroup}>
            ...
            <Text style={styles.total}>{totalPriceFourthFloorVoid.toLocaleString()} VNĐ</Text>
          </View>
        </View>
      </View>
...
export default FourthFloorVoid;
