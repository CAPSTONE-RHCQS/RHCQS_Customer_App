...
const FirstFloorVoid: React.FC = () => {
  ...
  const route = useRoute<RouteProp<ConstructionStackParamList, 'FirstFloorVoid'>>();
  const {Name} = route.params;

  const [areaFirstFloorVoid, setAreaFirstFloorVoid] = useState('');
  ...
  const constructionArea = areaFirstFloorVoid ? parseFloat(areaFirstFloorVoid) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceFirstFloorVoid = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaFirstFloorVoid');
      if (savedArea) {
        setAreaFirstFloorVoid(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaFirstFloorVoid', areaFirstFloorVoid);
    };
    saveArea();
  }, [areaFirstFloorVoid]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        <InputField
          name=""
          value={areaFirstFloorVoid}
          onChangeText={setAreaFirstFloorVoid}
        ...
        <View>
          ...
          <View style={styles.titleGroup}>
            ...
            <Text style={styles.total}>{totalPriceFirstFloorVoid.toLocaleString()} VNĐ</Text>
          </View>
        </View>
      </View>
