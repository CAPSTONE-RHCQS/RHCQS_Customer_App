...
const Rooftop: React.FC = () => {
  ...
  const route = useRoute<RouteProp<ConstructionStackParamList, 'Rooftop'>>();
  const {Name} = route.params;

  const [areaRooftop, setAreaRooftop] = useState('');
  ...
  const constructionArea = areaRooftop ? Number(areaRooftop) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceRooftop = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaPIT');
      if (savedArea) {
        setAreaRooftop(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaRooftop', areaRooftop);
    };
    saveArea();
  }, [areaRooftop]);
  ...
  const handleContinuePress = async () => {
    ...
    await storage.setItem('totalPriceRooftop', totalPriceRooftop.toString());
    await storage.setItem('areaRooftop', areaRooftop.toString());

    navigationContruction.navigate('ConstructionScreen', {
      totalPriceRooftop,
      areaRooftop: Number(areaRooftop),
      source: 'Sân thượng có mái che',
    });
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        ...
        <View style={styles.titleGroup}>
          ...
          <Text style={styles.total}>{totalPriceRooftop.toLocaleString()} VNĐ</Text>
        </View>
      </View>
