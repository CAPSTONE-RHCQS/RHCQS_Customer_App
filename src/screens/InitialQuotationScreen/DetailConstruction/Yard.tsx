...
const Yard: React.FC = () => {
  ...
  const route = useRoute<RouteProp<ConstructionStackParamList, 'Yard'>>();
  const {Name} = route.params;

  const [areaYard, setAreaYard] = useState('');
  ...
  const constructionArea = areaYard ? Number(areaYard) * coefficient : 0;
  const unitPrice = roughPackagePrice;
  const totalPriceYard = constructionArea * unitPrice || 0;

  useEffect(() => {
    const loadArea = async () => {
      const savedArea = await storage.getItem('areaPIT');
      if (savedArea) {
        setAreaYard(savedArea);
      }
    };
    loadArea();
  }, []);
  ...
  useEffect(() => {
    const saveArea = async () => {
      await storage.setItem('areaYard', areaYard);
    };
    saveArea();
  }, [areaYard]);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        ...
        <View style={styles.titleGroup}>
          ...
          <Text style={styles.total}>{totalPriceYard.toLocaleString()} VNĐ</Text>
        </View>
      </View>
